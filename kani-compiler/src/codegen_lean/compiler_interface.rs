...
//! This file contains the code necessary to interface with the Lean backend

use crate::args::ReachabilityType;
use crate::codegen_lean::LeanCtx;
...
// TODO: What is the use of MachineModel?
#[derive(Clone, Debug)]
...
// TODO: do we need the rounding modes of CBMC in Lean or even Boogie?
...
impl LeanCodegenBackend {
    pub fn new(queries: Arc<Mutex<QueryDb>>) -> Self {
        LeanCodegenBackend { queries }
    }
    ...
    ) -> (LeanCtx<'tcx>, Vec<MonoItem<'tcx>>) {
        let items = with_timer(
            || collect_reachable_items(tcx, starting_items),
            "codegen reachability analysis",
        );
        dump_mir_items(tcx, &items, &lean_file.with_extension("kani.mir"));
...
impl CodegenBackend for LeanCodegenBackend {
    ...
    fn print_version(&self) {
        println!("Kani-lean version: {}", env!("CARGO_PKG_VERSION"));
    }
    ...
    fn codegen_crate(
        ...
        let ret_val = rustc_internal::run(tcx, || {
            ...
            let mut results = LeanCodegenResults::new(tcx, reachability);
            match reachability {
                ...
                ReachabilityType::PubFns => {
                    ...
                    let model_path = base_filename.with_extension(ArtifactType::SymTabGoto);
                    let (lcx, items) = self.codegen_items(
                        tcx,
                        &local_reachable,
                        &model_path,
...
impl<'tcx> LeanCodegenResults<'tcx> {
